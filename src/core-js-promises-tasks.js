...
function getFirstResolvedPromiseResult(promises) {
  const promiseSolve = promises.map((promise) =>
    Promise.resolve(promise).catch(() => Promise.reject())
  );

  return Promise.race(promiseSolve);
}
...
function getAllResult(promises) {
  const result = promises.map((promise) => promise.catch(() => null));
